...
In this dataset, because we care about the Redfiled ratio that is the ratio of N:P. to able to get the ratio, we need to get concentration of N from NO3, NH4 and NO2 to get the concentration of N. For the concentration of P, we use the data from P-column. 
...
```{r}
bg_chem <- read_csv(here::here("raw data", "BGchem2008data.csv"))
```
...
Comment: The plot shows a positive relationship between phosphate and total nitrogen. As phosphate incrreases, total nitrogen increases as well. The points are not all linear, many samples at low P and low N, and another grapu at higher P with higer N, it is suggesting different conditions.
